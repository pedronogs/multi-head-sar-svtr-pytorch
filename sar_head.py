...
class SAREncoder(nn.Module):
    ...
    def __init__(self, enc_bi_rnn=False, enc_drop_rnn=0.1, enc_gru=False, d_model=512, d_enc=512, mask=True, **kwargs):
        ...
        kwargs = dict(
            input_size=d_model, hidden_size=d_enc, num_layers=2, dropout=enc_drop_rnn, bidirectional=enc_bi_rnn
        )
        if enc_gru:
    ...
    def forward(self, feat):
        ...
        feat_v = torch.transpose(feat_v, 2, 1)  # bsz * W * C
...
class BaseDecoder(nn.Module):
    ...
    def forward(self, feat, out_enc, label=None, train_mode=True):
        ...
        if train_mode:
            return self.forward_train(feat, out_enc, label)
...
class ParallelSARDecoder(BaseDecoder):
    ...
    def _2d_attention(self, decoder_input, feat, holistic_feat, valid_ratios=None):
        ...
        attn_query = attn_query.unsqueeze(3).unsqueeze(4)  # (bsz, seq_len + 1, attn_size, 1, 1)

        attn_key = self.conv3x3_1(feat)  # bsz * attn_size * h * w
        attn_key = attn_key.unsqueeze(1)  # bsz * 1 * attn_size * h * w

        attn_weight = torch.tanh(torch.add(attn_key, attn_query))  # bsz * (seq_len + 1) * attn_size * h * w

        bsz, T, c, h, w = attn_weight.shape
        attn_weight = attn_weight.view(bsz, T, h, w, c)  # bsz * (seq_len + 1) * h * w * attn_size
        ...
        attn_weight = attn_weight.view(bsz, T, c, h, w)
        ...
        if self.pred_concat:
            holistic_feat = holistic_feat.expand(-1, attn_feat.shape[1], -1)
    ...
    def forward_train(self, feat, out_enc, label):
        lab_embedding = self.embedding(label)  # bsz * seq_len * emb_dim
        out_enc = out_enc.unsqueeze(1)  # bsz * 1 * emb_dim

        in_dec = torch.concat((out_enc, lab_embedding), dim=1)  # bsz * (seq_len + 1) * C

        out_dec = self._2d_attention(in_dec, feat, out_enc)
...
class SARHead(nn.Module):
    ...
    def forward(self, feat, label):
        ...
        if self.training:
            final_out = self.decoder(feat, holistic_feat, label)
        else:
            final_out = self.decoder(feat, holistic_feat, label=None, train_mode=False)
